...
class SPLICE(nn.Module):
    ...
    def forward(self, image, text):
        ...
        """
        image, _ = self.encode_image(image)
    ...
    def encode_image(self, image):
        ...
        if self.return_weights and not self.return_cosine:
            return weights, clip_enc

        recon_image = self.recompose_image(weights)

        if self.return_weights and self.return_cosine:
            return (weights, torch.diag(recon_image @ image.T).sum()), clip_enc
        
        if self.return_cosine:
            return (recon_image, torch.diag(recon_image @ image.T).sum()), clip_enc
        
        return recon_image, clip_enc
