...
def load_splice_embeddings(
    ...
    if obj_embeddings_path is not None:
        ...
        with h5py.File(obj_embeddings_path, "r") as f:
            try:
                # for files that are encoded in the old format (i.e. with zero padding for all objects)
                ...
                # for files that are encoded in the new format (i.e. with variable number of objects)
                train_group = f["train_obj_embeddings"]
                # Load all datasets in group into a list
                train_obj_embeddings = [
                    torch.from_numpy(train_group[key][:])
                    for key in sorted(
                        train_group.keys(), key=lambda x: int(x.split("_")[-1])
                    )
                ]
                embedding_dim = train_obj_embeddings[0].shape[1]

                padded_embeddings = torch.stack(
                    [
                        F.pad(t, (0, 0, 0, num_objects_training - t.shape[0]))
                        for t in train_obj_embeddings
                    ]
                )
                train_obj_embeddings = padded_embeddings

                test_group = f["test_obj_embeddings"]
                # Load all datasets in group into a list
                test_obj_embeddings = [
                    torch.from_numpy(test_group[key][:])
                    for key in sorted(
                        test_group.keys(), key=lambda x: int(x.split("_")[-1])
                    )
                ]
                embedding_dim = test_obj_embeddings[0].shape[1]

                padded_embeddings = torch.stack(
                    [
                        F.pad(t, (0, 0, 0, num_objects_training - t.shape[0]))
                        for t in test_obj_embeddings
                    ]
                )
                test_obj_embeddings = padded_embeddings
        ...
        train_id_to_index = {
            img_id.item(): idx for idx, img_id in enumerate(train_obj_img_ids)
        }
        test_id_to_index = {
            img_id.item(): idx for idx, img_id in enumerate(test_obj_img_ids)
        }

        # Step 2: Use that mapping to get the correct indices for reordering
        train_aligned_indices = [
            train_id_to_index[img_id.item()] for img_id in train_img_ids
        ]
        test_aligned_indices = [
            test_id_to_index[img_id.item()] for img_id in test_img_ids
        ]

        # Step 3: Convert to a tensor of indices and align the embeddings
        train_aligned_indices_tensor = torch.tensor(
            train_aligned_indices, dtype=torch.long
        )
        train_obj_embeddings = train_obj_embeddings[train_aligned_indices_tensor]
        test_aligned_indices_tensor = torch.tensor(
            test_aligned_indices, dtype=torch.long
        )
        test_obj_embeddings = test_obj_embeddings[test_aligned_indices_tensor]

        train_obj_embeddings, val_obj_embeddings = torch.split(
            train_obj_embeddings, [train_size, val_size]
        )
        avg_num_obj = torch.mean(
            torch.clamp(train_num_obj, min=0, max=num_objects_training).float()
        )
    else:
    ...
    train_embeddings, val_embeddings = torch.split(
        train_embeddings, [train_size, val_size]
    )
    train_clip_embeddings, val_clip_embeddings = torch.split(
        train_clip_embeddings, [train_size, val_size]
    )
...
def train_model(
    ...
    if task_type == "multiclass":
        if dataset == "cocologic":
            class_counts = [36, 56, 2965, 4188, 24, 8252, 4954, 3570, 279, 169]
    ...
    for epoch in range(num_epochs):
        ...
        for embeddings, clip_embeddings, obj_embeddings, img_ids, labels in tqdm(
            train_loader
        ):
            ...
            if use_obj:
                ...
                embeddings = obj_enc_aggregation(
                    aggregation, obj_embeddings, embeddings, only_objects
                )

            ### === First pass: Classification Loss + Prediction Loss === ###
        ...
        total = 0
        if task_type == "multilabel":
            correct_per_class = torch.zeros(labels.shape[1]).to(device)
            collect_labels = []
            collect_preds = []
        else:
            correct = 0
        ...
        with torch.no_grad():  # No gradients needed for validation
            ...
            if task_type == "multilabel":
                ...
                map_score = average_precision_score(
                    torch.cat(collect_labels).cpu(),
                    torch.cat(collect_preds).cpu(),
                    average="macro",
                )
    ...
    with torch.no_grad():
        for embeddings, clip_embeddings, obj_embeddings, img_ids, labels in tqdm(
            ...
            if no_concepts:
                clip_embeddings = clip_embeddings.to(device)
                outputs = model(clip_embeddings)
                print("Use clip embeddings for testing.")
            else:
                outputs = model(embeddings)
    ...
    if task_type == "multilabel":
        ...
        balanced_acc = balanced_accuracy_score(
            torch.cat(collect_labels).cpu(), torch.cat(preds).cpu()
        )
        per_class_acc = recall_score(
            torch.cat(collect_labels).cpu(), torch.cat(preds).cpu(), average=None
        )
...
if __name__ == "__main__":
    ...
    base_embeddings_path = os.path.join(
        args.encoding_dir,
        args.dataset,
        args.model,
    ...
    ), f"SpLiCE embeddings file {base_embeddings_path} does not exist. Please generate the SpLiCE embeddings first."
    if args.use_object_concepts:
        ...
        assert os.path.exists(
            file_path
        ), f"Object encodings file {file_path} does not exist. Please generate the object encodings first."
        # create datasets with object_encodings
        train_dataset, val_dataset, test_dataset, encoding_dim, avg_num_obj = (
            load_splice_embeddings(
                base_embeddings_path,
                file_path,
                prediction_level=args.prediction_level,
                num_objects_training=args.num_objects_training,
                debug=args.dataset == "cocologic",
            )
        )
    else:
    ...
    num_classes = (
        CLASS_NUMBERS[args.dataset][0]
        if args.prediction_level == "super"
        else CLASS_NUMBERS[args.dataset][1]
    )

    if args.no_concepts:
        model = MLPPredictor(
            num_classes=num_classes,
            implicit_encoding_dim=512,
            device=args.device,
        )
    else:
